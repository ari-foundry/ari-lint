...
usage, missing Ari value usage, and `--list-rules` comparison signals, creates
...
for `--help`, an unknown option, missing `--config` value, missing `--rule`
value, missing `--ari` value, `--list-rules`, explicit `--config`, CLI `--rule`,
