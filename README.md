...
`--help`, unknown-option usage, and `--list-rules` comparison signals, creates
tiny temporary trailing-whitespace, missing-final-newline, clean, config, and
...
file-path hit counts, and line/column presence. It includes report-only coverage
for `--help`, an unknown option, `--list-rules`, explicit `--config`, CLI
`--rule`, discovered `ari-lint.rules`, and multi-file invocations. Differences
