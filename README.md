...
`--help` and `--list-rules` comparison signals, creates tiny temporary
...
counts, and line/column presence. It includes report-only coverage for
`--help`, `--list-rules`, explicit `--config`, CLI `--rule`, discovered
...
`--rule` text and missing `--config` or `--rule` values only for the current
short stderr summary. Focused diagnostic smoke checks assert current `ruleCode`,
`severity`, `message`, `filePath`, `line`, and `column` fields for
`lint/trailing-whitespace` and `lint/missing-final-newline`, plus multi-file JSON
diagnostics, a clean plus
