...
usage, invalid `--rule` severity usage, and `--list-rules` and JSON list-rules
comparison signals, then creates tiny temporary
...
`--config`, invalid `--config`, malformed `--rule`, invalid `--rule` severity,
CLI `--rule`, discovered `ari-lint.rules`, and multi-file invocations.
