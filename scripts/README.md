...
JSON smoke commands against temporary files to check config-provided severity
and CLI `--rule` precedence.
...
CI, config discovery, new lint semantics, or compatibility claims. It checks
only the current JSON rule code and severity fields for the temporary
