...
native-rule parity subset, an initial strict explicit-config subset, one
deterministic strict compiler-boundary case, exact standalone/reference
list-rules contracts, and checksum-pinned Ari `v0.1.0` compiler-smoke CI. The
source-result JSON,
...
CLI diagnostic vector collection added /
...
- [x] Replace the single-diagnostic JSON placeholder with internal field
      serialization for filePath, line, column, nullable endLine/endColumn,
      severity, ruleCode, and message, including common string escaping,
      without writing stdout/stderr, wiring CLI output, serializing diagnostic
...
- [x] Add an internal CLI diagnostic vector collection path for explicit
      caller-provided tokens and parsed source-file input, pushing full
      internal diagnostics into a caller-provided vector while returning
      existing command-result count and exit-code data, without changing
      main-facing output, writing full diagnostic arrays, serializing JSON,
      discovering config files, reading config files, traversing directories,
      invoking the compiler, executing `ari --check`, calling `tools/lint`, or
...
- [x] Wire the main-facing CLI parse problem path to write a short usage-error
      ...
      parse diagnostics, tests, and parity checks remain future
      work.
- [x] Wire the main-facing CLI help path to write concise help text to stdout
      through the verified stdout adapter, without config discovery, reading
      config files, traversing directories, invoking the compiler, executing
...
- [x] Add Ari compiler invocation plan in
      ...
      Compiler-backed smoke CI and one deterministic strict compiler-boundary
      case now exist; broader compiler-boundary parity and a compatibility
...
- [x] Plan parity testing against current `tools/lint`;
      ...
      ordered multi-file, and duplicate JSON results. One deterministic strict
      compiler-boundary case gates exact `SOURCE --check` child argv and
      combined JSON and human diagnostics. An initial strict config subset
      gates explicit severity, `off`, and CLI-last behavior. Strict CLI,
      broader config/compiler-boundary, and CI parity remain open.
- [x] Add source-only parity runner skeleton in `src/parity.ari`; it remains a
      ...
      strict native, initial config, and initial compiler-boundary
      fixture/golden subsets. Ari-source runner execution, broader config and
